...
var arrayDiNumeriCasuali = myArrayNumeriCasuali();
console.log(arrayDiNumeriCasuali);

/* ESERCIZIO 3 (filter)
  Scrivi una funzione per ricavare solamente i valori PARI da un array composto da soli valori numerici
*/
let arrayPari= arrayDiNumeriCasuali;

function filtroPari(element){
  return element%2 ==0;
}

let numeropari= arrayPari.filter(filtroPari);
...
let sommanumeri= arrayDiNumeriCasuali;
function sommaDiNumeri (sommanumeri){
  let somma =0;
  sommanumeri.forEach(function(numero){
    somma+=numero
  });
return somma;
  }

  const risultato = sommaDiNumeri(sommanumeri);
console.log("La somma dei numeri nell'array è:", risultato);

/* ESERCIZIO 5 (reduce)
  Scrivi una funzione per sommare i numeri contenuti in un array
*/
let sommareduce = arrayDiNumeriCasuali;
function sommaConReduce(sommareduce){
  const  somma = sommareduce.reduce(function(accumulator, currentValue) {
...
const risultato2 = sommaConReduce(sommareduce);
console.log("La somma dei numeri nell'array è:", risultato2);
...
let incrementareArray= arrayDiNumeriCasuali;
var n = Math.floor(Math.random() * 10) + 1;

function incrementaArray(incrementareArray, n) {
  // Utilizziamo il metodo map() per creare un nuovo array con i valori incrementati di n
  const arrayIncrementato = incrementareArray.map(function(numero) {
      return numero + n;
  });
...
const arrayIncrementato = incrementaArray(incrementareArray, n);
console.log("Array originale:", incrementareArray);
...
function trovaFilmPiuVecchio(movies) {
  let filmPiuVecchio = movies[0]; // Iniziamo assumendo che il primo film sia il più vecchio

  movies.forEach(function(film) {
      // Convertiamo gli anni in numeri e confrontiamo
      const annoCorrente = parseInt(film.Year);
      const annoPiuVecchio = parseInt(filmPiuVecchio.Year);

      if (annoCorrente < annoPiuVecchio) {
          filmPiuVecchio = film; // Se troviamo un film più vecchio, lo assegniamo a filmPiuVecchio
      }
  });
